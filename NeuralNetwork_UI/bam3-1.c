...
/*******************************************************************
File Name:	BAM3-1.c      let hidden layer value in [-1, 1]
Created:	8/7/97
Revised:
...
#define		  H	110 	/* dim. of the hidden layer */

#define		  M	66      /* number of pattern pairs */
#define		 NC     500	/* number of learning cycles */
...
void	update_by_A(int error_A[N1], int k)
{
	...
	for(j=0; j<H; j++){
	  ...
	  delta[j] *= (1+Hid[j])*(1-Hid[j])*ALPHA/2;
	}
	for(i=0; i<N1; i++) for(j=0; j<H; j++)
