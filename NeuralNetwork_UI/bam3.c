...
Revised:
Note:		
		This code includes the learning function and recall function.

		(1) BAM with 3 layers (with a nonlinear hidden layer).
...
#define		  H	40 	/* dim. of the hidden layer */

#define		  M	26	/* number of pattern pairs */
#define		 NC     1000	/* number of learning cycles */
				/* one cycle covers all pattern pairs */
#define	       RATE      0.02    /* learning rate */

#define		DATA_FILE	"pp.0"
...
void	read_data()
{
	...
	for(k=0; k<M; k++){
	  file_name[3] = 'a'+k;	/* set pattern file name */
...
{
	register int	i, j;
	float		delta[H]; /* error propagated to the H-layer */

	for(j=0; j<H; j++){
	  delta[j] = 0;
	  for(i=0; i<N1; i++)   delta[j] = V[i][j]*error_A[i];
	  delta[j] *= Hid[j]*(1-Hid[j]);
	}
	for(i=0; i<N1; i++) for(j=0; j<H; j++)
	  V[i][j] += RATE*error_A[i]*Hid[j];

	for(i=0; i<H; i++){
	  for(j=0; j<N2; j++)  W[i][j] += RATE*delta[i]*B[k][j];
	  Theta[i] += RATE*delta[i];
	}
}
...
void	update_by_B(int error_B[N2], int k)
{
	...
	for(j=0; j<H; j++){
	  delta[j] = 0;
	  for(i=0; i<N2; i++)   delta[j] = W[j][i]*error_B[i];
	  delta[j] *= Hid[j]*(1-Hid[j]);
	}
	for(i=0; i<H; i++) for(j=0; j<N2; j++)
	  W[i][j] += RATE*Hid[i]*error_B[j];

	for(i=0; i<H; i++){
	  for(j=0; j<N1; j++)  V[j][i] += RATE*A[k][j]*delta[i];
	  Theta[i] += RATE*delta[i];
	}
}
...
void	learn()
{
	...
	for(cycle=0; cycle<NC; cycle++){
	  ...
	  for(k=0; k<M; k++){
		...
		{   error_B[j] = B[k][j];	All_Positive = 0; }
	      else  error_B[j] = 0;
	    }
	    update_by_B(error_B, k);

	    output_from_A(output_A, B[k]);

	    for(i=0; i<N1; i++)
	      if((output_A[i]*A[k][i])<=0)
		{   error_A[i] = A[k][i];	All_Positive = 0;}
	      else  error_A[i] = 0;
	    update_by_A(error_A, k);
	  }
	  if(All_Positive){
