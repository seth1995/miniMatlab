...
{
								 if (!HaoYuWidget)
																HaoYuWidget = new QMainWindow(this);
								 if (!imageLabel)
								...
								HaoYuWidget->setCentralWidget(scrollArea);
								// QWebView *view = new QWebView(HaoYuWidget);
								...
								//action
								openFileAction = new QAction(QIcon(":/images/cut.png"), tr("Open"), HaoYuWidget);
								connect(openFileAction, SIGNAL(triggered()), this, SLOT(HaoYuOpen()));
								//
								runScriptAction = new QAction(QIcon(":/images/cut.png"), tr("Run"), HaoYuWidget);
								//connect(runScriptAction, SIGNAL(triggered()), this, SLOT());
								//end of action
								//UI part
								showP0Action = new QAction(QIcon(":/images/cut.png"), tr("Show P0"), HaoYuWidget);

								connect(showP0Action, SIGNAL(triggered()), this, SLOT(ShowImg0()));
								showP1Action = new QAction(QIcon(":/images/cut.png"), tr("Show P1"), HaoYuWidget);
								connect(showP1Action, SIGNAL(triggered()), this, SLOT(ShowImg1()));
								showP2Action = new QAction(QIcon(":/images/cut.png"), tr("Show P2"), HaoYuWidget);
								connect(showP2Action, SIGNAL(triggered()), this, SLOT(ShowImg2()));
								showP3Action = new QAction(QIcon(":/images/cut.png"), tr("Show P3"), HaoYuWidget);
								connect(showP3Action, SIGNAL(triggered()), this, SLOT(ShowImg3()));
								showP4Action = new QAction(QIcon(":/images/cut.png"), tr("Show P4"), HaoYuWidget);
								connect(showP4Action, SIGNAL(triggered()), this, SLOT(ShowImg4()));

								showHtmlAction = new QAction(QIcon(":/images/cut.png"), tr("Show HTML"), HaoYuWidget);
								connect(showHtmlAction, SIGNAL(triggered()), this, SLOT(ShowHtml()));

								HaoYuFileMenu=HaoYuWidget->menuBar()->addMenu(tr("&File"));
								HaoYuFileMenu->addAction(openFileAction);
								HaoYuRunMenu=HaoYuWidget->menuBar()->addMenu(tr("&Run"));
								HaoYuRunMenu->addAction(runScriptAction);
								HaoYuShowMenu=HaoYuWidget->menuBar()->addMenu(tr("&Show"));
								HaoYuShowMenu->addAction(showP0Action);
								HaoYuShowMenu->addAction(showP1Action);
								HaoYuShowMenu->addAction(showP2Action);
								HaoYuShowMenu->addAction(showP3Action);
								HaoYuShowMenu->addAction(showP4Action);
								HaoYuShowMenu->addAction(showHtmlAction);
								HaoYuWidget->menuBar()->addSeparator();

								//end of UI

								QVBoxLayout *layout = new QVBoxLayout(HaoYuWidget);
								//layout->addWidget(view);

								HaoYuWidget->show();
								HaoYuWidget->resize(1024,800);
								HaoYuWidget->setWindowTitle(tr("Hao Yu"));
}
void MainWindow::HaoYuOpen()
{
								QString fileName = QFileDialog::getOpenFileName(this,
																																																								tr("Open File"), QDir::currentPath(),tr("Dataset (*.csv)\n"));
								if (!fileName.isEmpty()) {
																//QImage image(fileName);
																// if (image.isNull()) {
																//   QMessageBox::information(this, tr("Solver"),
																...
																QProcess *process=new QProcess;
																process->start("Rscript", QStringList() << "/code/R/test.R"<<fileName);
																connect(process, SIGNAL(finished(int,QProcess::ExitStatus)), this, SLOT(on_jobFinished()));
...
{
	QMainWindow *tmpWindow=new QMainWindow(this);

	QWebView *view = new QWebView(HaoYuWidget);
	tmpWindow->setCentralWidget(view);
 	tmpWindow->resize( QSize( 800,600 ));
  view->load(QUrl("file:///code/NN/Main/temp.html"));
	tmpWindow->setWindowTitle(tr("Visualize the route result"));
	tmpWindow->show();
}
void MainWindow::ShowImg0()
{
	QString imgName("./result/p0.png");
...
void MainWindow::on_jobFinished()
{
								cout<<"Finished"<<endl;
								QImage image("./result/p0.png");
								imageLabel->setPixmap(QPixmap::fromImage(image));
								imageLabel->adjustSize();
